...
class Product
{
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                if ($row['category_id'] == $category) {
                    ...
                    echo '
                        <div class="col-lg-3">
                            <form action="../../controllers/handle_cart.php" method="post" class="mt-3 mb-3">
                                <div class="card">
                                    <img src="' . dir_root_url . '/' . $image . '" class="card-img-top" alt="" style="height: 250px;">
    ...
    public function addProduct($product_id, $product_name, $category_id, $product_img, $product_desc, $product_price)
    {
        ...
        if (DB::connect()->query($sql) === TRUE) {
            if ($category_id == 1) {
                header('Location:http://localhost/admin_cf/?table=coffee');
            } elseif ($category_id == 2) {
                header('Location:http://localhost/admin_cf/?table=cake');
            }
        } else {
    ...
    public function compareID($product_id)
    {
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<script>window.location.href = "http://localhost/admin_cf/views/view_form/form_add_product.php";</script>';
            die();
        } 
    }
}
