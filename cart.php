...
class Cart
{
    ...
    public static function addToCart($productName, $price, $categoryID)
    {
        ...
        if ($categoryID == 1) {
            header('Location:http://localhost/admin_cf/views/view_cart/menu_product.php?list=coffee_list');
        } elseif ($categoryID == 2) {
            header('Location:http://localhost/admin_cf/views/view_cart/menu_product.php?list=cake_list');
        }
    }
    ...
    public static function removeFromCart($productName)
    {
        foreach ($_SESSION['cart'] as $key => $value) {
            if ($value['product_name'] == $productName) {
                ...
                header('Location:http://localhost/admin_cf/views/view_cart/my_cart.php');
            }
        }
    }
    ...
    public static function makePayment($tableNumber, $note, $customerPhone)
    {
        ...
        if (DB::connect()->query($sql)) {
            ...
            echo '<script>window.location.href = "http://localhost/admin_cf/views/view_cart/payment.php";</script>';
        } else {
